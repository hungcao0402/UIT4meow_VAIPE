...
def set_environment(args):
    ...
    dataset = DataCustom(istrain=True, root=args.input_root, root_ocr=args.ocr_root, data_size=args.data_size, return_index=True)
...
def main_test(args, tlogger):
    ...
    with torch.no_grad():
        for batch_idx, (ids,path, images,onehots,labels) in tqdm(enumerate(loader)):
            ...
            outs  = model(images,onehots)
            tmp_score = torch.softmax(outs["ori_out"], dim=-1)
